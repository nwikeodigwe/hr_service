...
def index():
    session = db.session
    employees = session.query(Employee).all() # ORM performs SELECT query
    result = []
    for e in employees:
...
def organiation(id: int):
    employee = Employee.query.filter_by(id=id).first_or_404()
    o = Organization.query.filter_by(id=employee.organization_id).first_or_404()
...
def update(id: int):
    ...
    #Find the user
    e = Employee.query.filter_by(id=id).first_or_404()
